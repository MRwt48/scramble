const moveDw = (
  ...
  for (let m = 0; m < wide; m++) {
    ...
    faceFront[cubeSize - 1 - m] = faceLeft[cubeSize - 1 - m].map(
      (block: string) => block
    );
    faceLeft[cubeSize - 1 - m] = faceBack[cubeSize - 1 - m].map(
      (block: string) => block
    );
    faceBack[cubeSize - 1 - m] = faceRight[cubeSize - 1 - m].map(
      (block: string) => block
    );
    faceRight[cubeSize - 1 - m] = tempArr.map((block: string) => block);
  }
